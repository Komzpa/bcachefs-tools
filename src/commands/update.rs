...
fn require_reconcile_complete(status: reconcile::ReconcileWaitStatus) -> Result<()> {
    match status {
        reconcile::ReconcileWaitStatus::Completed => Ok(()),
        reconcile::ReconcileWaitStatus::Cancelled => Err(anyhow::anyhow!(
            "reconcile wait cancelled while work remains"
        )),
    }
}
...
fn cmd_update(cli: UpdateCli) -> Result<()> {
    ...
    require_reconcile_complete(wait_ret?)?;
    reset_ret?;
...
mod tests {
    ...
    #[test]
    fn cancelled_reconcile_is_an_error() {
        assert!(require_reconcile_complete(reconcile::ReconcileWaitStatus::Cancelled).is_err());
    }
}
