...
static CLOSURE_CALLBACK(journal_write_done)
{
	...
	while ((seq = last_uncompleted_write_seq(j, seq_wrote))) {
		...
		if (w->wait.list.first != JOURNAL_BUF_NOFLUSH) {
			struct closure_waitlist	wait = {{ xchg(&w->wait.list.first, JOURNAL_BUF_NOFLUSH) }};
...
static int __should_flush(struct journal *j, struct journal_buf *w, u64 seq)
{
	...
	if (must_flush && j->flushes_outstanding > 1) {
		...
		if (next && !journal_buf_must_not_flush(next)) {
			/*
...
static int should_flush(struct journal *j, struct journal_buf *w, u64 seq)
{
	...
	if (!ret &&
	    cmpxchg(&w->wait.list.first, NULL, JOURNAL_BUF_NOFLUSH) > JOURNAL_BUF_NOFLUSH)
		ret = 1;
	return ret;
}
